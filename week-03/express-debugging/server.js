...
app.get('/name', (req, res) => {
    const helloToName = myModule.sayHello("Nick");
    res.send(helloToName);
});

// http://localhost:8080/nameq?personName=Jon
app.get('/nameq', (req, res) => {
    const personName = req.query.personName || "Nobody";
    const helloToName = myModule.sayHello(personName);
    res.send(helloToName);
});

// http://localhost:8080/namep/Frank
app.get('/namep/:personName', (req, res) => {
    const personName = req.params.personName || "Nobody";
    const helloToName = myModule.sayHello(personName);
    res.send(helloToName);
});
